program hdf2xml
  ...
  write (*,*) "-------------------------"
  write (*,*) "  2013, Thomas ENGELS    "
  write (*,*) "-------------------------"
  
  !---------------------------------------------------------------------
  ! read in LIST OF scalar prefixes. 
  ! note it is mandatory to follow file naming convention.
  ! for a file mask_00010.h5 "mask" is in this file
  !---------------------------------------------------------------------  
  io_error=0
  ...
  !---------------------------------------------------------------------
  ! read in LIST OF vector prefixes. 
  ! note vectors have 3 components and one basename. this file contains the basename only
  ! i.e. you save usx_00010.h5 usy_00010.h5 usz_00010.h5
  ! the basename is "us"
  ! note it is mandatory to follow file naming convention.
  !---------------------------------------------------------------------
  io_error=0
  ...
  ! we need resolution for BEGIN_FILE!!!
  ...
  do idt = 1, ndt  
    ...
    !-------------------------
    do iscal = 1, nscal
      call WriteScalar( trim(adjustl(timesteps(idt))), trim(adjustl(prefixes_scalar(iscal))),&
    ...
    !-------------------------
    do ivec = 1, nvec  
      call WriteVector( trim(adjustl(timesteps(idt))), trim(adjustl(prefixes_vector(ivec))),&
                        nx, ny, nz, xl, yl ,zl, time )
    enddo  
    !-------------------------
    ! time step footer
...
subroutine Fetch_attributes( filename, dsetname,  nx, ny, nz, xl, yl ,zl, time )
  ...
  ! note:
  !           the file must contain the dataset (lol)
